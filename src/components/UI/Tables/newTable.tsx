...
interface JobType {
  ...
  jobSpecialityId: string;
  jobCategoryId: string;
  jobCareerLevelId: string;
  jobEmploymentTypeId: string;
  jobWorkPlace: "REMOTE" | "ON_SITE" | "HYBRID"; // Restricting the possible values
  gender: "MALE" | "FEMALE" | "ANY"; // Restricting the possible values
  minAge: number;
  maxAge: number;
  educationLevel: "HIGH_SCHOOL" | "BACHELORS" | "MASTERS" | "PHD"; // Restricting the possible values
  ...
  salaryCurrency: "USD" | "EUR" | "GBP"; // Restricting the possible values
...
const OverveiwJobTable: React.FC<OverviewEmployersTableProps> = ({
  ...
  const [industriesData, setIndustriesData] = useState<IndustryCategoryType[]>(
    [],
  );
  ...
  const API_GET_CATEGORY =
    "http://34.70.58.31/api/v1.0.0/admin/sys-configurations/category";
  ...
  const columns: GridColDef[] = [
    ...
    },
    {
      ...
      sortable: false,
      flex: 1,
      renderCell: (params) => handleState(params.row.state),
    },
